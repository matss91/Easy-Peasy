...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
            var cantp=movies.length
            var peliculas=titulos()
			...
			res.end("Bienvenidos a DH Movies el mejor sitio para encontrar las mejores"+"películas, incluso mucho mejor que Netflix, Cuevana y PopCorn."
				+' numero de peliculas:'+cantp+"\n lista:"+peliculas+"Pié de página: Recordá que podés visitar las secciones:\n"+
			    "i.   En Cartelera\n"+
				"ii.  Más Votadas\n"+
				"iii. Sucursales\n"+
				"iv.  Contacto\n"+
		...
		case '/sucursales':
			...
			res.write("cantidad de salas: "+theaters.length+"\n")
			res.write(salas())
		res.end('Sucursales');
			break;
		case '/contacto':
		...
		res.write("total de preguntas:"+theaters.length)
res.write(preguntas())
		res.end();
			break;
		default:
...
pelis[i]=(movies[i].original_title+"\n")


}
var ordenado=pelis.sort()

coma=ordenado.toString()
for(i=0;i<20;i++){
coma=coma.replace(patron,nuevovalor)
...
var cart=new Array();
for(i=0;i<movies.length;i++){
	cart[i]=(movies[i].original_title+":\n"+movies[i].overview)
}
...
function destacadas(){
	var valorados=movies.filter(function(i){
		
		return i.vote_average>7
	...
	for(i=0;i<valorados.length;i++){
		cart[i]=("\n"+valorados[i].title+"\n"+"rating:"+valorados[i].vote_average+"\n"+valorados[i].overview)
	contador=contador+valorados[i].vote_average

	}
	
	return cart.toString()+"\n"+contador/valorados.length
	...
	function salas(){
		...
		for(i=0;i<theaters.length;i++){
			sucursales[i]=(theaters[i].name+theaters[i].address+theaters[i].description+"\n\n")
		}
